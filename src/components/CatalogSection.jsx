...
const CatalogSection = ({ catalogByType = {}, carTypes = [], salesInfo = {} }) => {
  ...
  const allCars = selectedType === 'All'
    ? Object.values(catalogByType).flat().filter(car => car && car.id && car.slug && car.image_url)
    : (catalogByType[selectedType] || []).filter(car => car && car.id && car.slug && car.image_url);

  // Ensure filteredTypes is an array
  ...
  const handleContactClick = (e, car) => {
    e.stopPropagation();
    const phone = salesInfo.phone || '6281234567890'; // Fallback phone
    const name = salesInfo.name || 'Sales Toyota'; // Fallback name
    window.open(
      `https://wa.me/${phone}?text=Halo%20${encodeURIComponent(name)},%20saya%20tertarik%20dengan%20${encodeURIComponent(car.name)}%20${encodeURIComponent(car.variant)}`,
      '_blank',
      'noopener,noreferrer'
    );
  };
        ...
        {JSON.stringify({
          ...
          "itemListElement": allCars.map((car, index) => ({
            ...
            "item": {
              ...
              "image": car.image_url ? `${baseUrl}${car.image_url}` : placeholderImage,
              ...
              "offers": {
                ...
                ...(car.validUntil && { "validThrough": car.validUntil })
              }
            }
          }))
        })}
          ...
          <div key={type} className="mb-12 md:mb-16">
            ...
            {allCars.length === 0 ? (
              <div className="text-center text-gray-600 py-8">
                Tidak ada mobil tersedia untuk tipe {type}.
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="relative p-4 sm:p-5 md:p-6">
                      ...
                      <div className="flex flex-nowrap gap-2">
                        <motion.button
                          onClick={() => handleDetailClick(car.slug)}
